...
pub struct Params<'a> {
    ...
    pub token_cache_path: &'a Path,
}

pub async fn save_secret(path: &Path, app_secret: &ApplicationSecret) -> anyhow::Result<()> {
    let json = serde_json::to_string_pretty(app_secret)?;
    tokio::fs::write(path, &json).await?;
    Ok(())
}

pub async fn load_secret(path: &Path) -> anyhow::Result<ApplicationSecret> {
